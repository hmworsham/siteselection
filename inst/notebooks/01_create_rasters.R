...
writeRaster(dem1000, file.path(config$dat_int, 'usgs_dem_1000m.tif'), overwrite=T)
writeRaster(dem100, file.path(config$dat_int, 'usgs_dem_100m.tif'), overwrite=T)
writeRaster(dem30, file.path(config$dat_int, 'usgs_dem_30m.tif'), overwrite=T)

# slope
slope <- terrain(dem100, v = 'slope', unit = 'degrees')
names(slope) <- 'slope_100m'
writeRaster(slope, file.path(config$dat_int, 'usgs_slope_100m.tif'), overwrite=T)
#slope <- NULL

# aspect
aspect100 <- terrain(dem100, v = 'aspect', unit = 'degrees')
names(aspect100) <- 'aspect_100m'
writeRaster(aspect100, file.path(config$dat_int, 'usgs_aspect_100m.tif'), overwrite=T)
#aspect100 <- NULL

# cos(aspect)
caspect <- cosaspect(aspect100, unit='deg')
names(caspect) <- 'cosaspect_100m'
writeRaster(caspect, file.path(config$dat_int, 'usgs_cosaspect_100m.tif'), overwrite=T)

# sin(aspect)
saspect <- sinaspect(aspect100, unit='deg')
names(saspect) <- 'sinaspect_100m'
writeRaster(saspect, file.path(config$dat_int, 'usgs_sinaspect_100m.tif'), overwrite=T)

# Folded aspect
faspect <- fold.aspect(aspect100, fold=205)
names(faspect) <- 'usgs_205faspect_100m'
writeRaster(faspect, file.path(config$dat_int, 'usgs_205faspect_100m.tif'), overwrite=T)

# Eastness
eastness <- facing(slope, aspect100, focal=90, unit='deg')
names(eastness) <- 'eastness_100m'
writeRaster(eastness, file.path(config$dat_int, 'usgs_eastness_100m.tif'), overwrite=T)

# Adjusted southness
asouthness <- facing(slope, aspect100, focal=205, unit='deg')
names(asouthness) <- '205adjsouthness_100m'
writeRaster(asouthness, file.path(config$dat_int, 'usgs_205adjsouthness_100m.tif'), overwrite=T)

# THL
thl <- thl(38.95, aspect100, slope, unit='deg', fold=205, equation=3)
names(thl) <- 'heatload_100m'
writeRaster(thl, file.path(config$dat_int, 'usgs_heatload_100m.tif'), overwrite=T)
...
writeRaster(curv10, file.path(config$dat_int, 'usgs_curvature_10m.tif'), overwrite=T)
writeRaster(curv100, file.path(config$dat_int, 'usgs_curvature_100m.tif'), overwrite=T)
...
writeRaster(tpi2000, file.path(config$dat_int, 'usgs_tpi_2000m.tif'), overwrite=T)

# TWI
twi100 <- twi(dem100, calc.res=100, out.res=100, fill.sinks=T, deg=0.1)
names(twi100) <- 'twi_100m'
writeRaster(twi100, file.path(config$dat_int, 'usgs_twi_100m.tif'), overwrite = T)

twi1000 <- twi(dem100, calc.res=1000, out.res=100, fill.sinks=T, deg=0.1)
names(twi1000) <- 'twi_1000m'
writeRaster(twi1000, file.path(config$dat_int, 'usgs_twi_1000m.tif'), overwrite = T)

# upslope contributing area
uca100 <- uca(dem100, calc.res=100, out.res=100, fill.sinks=T, deg=0.1)
names(uca100) <- 'uca_100m'
writeRaster(uca100, file.path(config$dat_int, 'usgs_uca_100m.tif'), overwrite=T)

uca1000 <- uca(dem100, calc.res=1000, out.res=100, fill.sinks=T, deg=0.1)
names(uca1000) <- 'uca_1000m'
writeRaster(uca1000, file.path(config$dat_int, 'usgs_uca_1000m.tif'), overwrite=T)
